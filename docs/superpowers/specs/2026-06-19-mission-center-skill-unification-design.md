...
- Council 會依任務動態選擇專業視角，並在重大分歧時呈現取捨。
...
- 依任務動態選擇 3 至 6 個相關視角，例如產品、領域研究、架構、資安、驗證、法務智財、營運、UX 與成本。
- 預設採混合呈現：一般情況只輸出共識、盲點與建議；架構、安全、成本、範圍或法務存在重大分歧時，才展開各方立場與取捨。
- Council 是主 Agent 的觀點模擬，不等於自動啟動子 Agent。
- 只有工作可獨立平行、需要獨立驗證且使用者明確同意時，才派遣真正的子 Agent；每批完成後先收回，再決定是否開下一批。
- Council 結果應改變問題、方案或任務設計，不輸出沒有決策價值的角色台詞。
...
- Council 一般情境：只輸出共識、盲點與建議，不展開冗長角色台詞。
- Council 重大分歧：呈現相關專家立場、取捨與推薦。
