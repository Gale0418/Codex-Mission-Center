...
| MC-032 | Optional Persistent Project Map | Experiment |  | P1 | Backlog | Codex | MC-026, MC-037 | 等 v0.3 續航契約完成後再設計 html/json/lock | 跨語言 spike、fingerprint tests、人工 review | 5 | codemap, experiment, visualization | 先確保接手不迷路；與 RuntimeState 分離 |
...
| MC-044 | Codex CLI 0.147 Agent Plugin Compatibility Spike | Experiment |  | P1 | Backlog | Codex | MC-043 | 比對 portable plugin catalog 與既有 publisher，先證明可回退再決定汰除範圍 | 官方 CLI 安裝／搜尋／更新／離線 fallback matrix | 2 | plugin, compatibility, research | 不改 tasks.md 或 repo-local memory 核心；不得憑新聞移除相容層 |
...
| MC-046 | OWO+ v0.4 Causal Continuity and Evolution Loop | Epic |  | P0 | In Progress | Codex | MC-045 | 執行 MC-051 完整驗證、審查與發布閘門 | 端到端換班 smoke、Shift-Loss eval、full regression、Doctor、publish verify | 13 | execution, memory, evolution | 保留 tasks.md 唯一 lifecycle truth；昂貴能力按證據啟用 |
...
| MC-051 | OWO+ v0.4 Review, Compatibility and Release | Task | MC-046 | P0 | Review | Codex | MC-048, MC-049, MC-050 | 等待 CodeRabbit 上傳同意與 critic_full 總額／席次／工具／時限預算核准 | full unittest、Doctor、CodeRabbit／Critic gate、publish verify | 2 | review, release, closeout | Antigravity 全工作區稽核後修正 5 類契約缺口；本機 246 tests＋74 subtests、Doctor、Skill／Plugin validators、publish dry-run、diff check 全部通過；外部審查與正式發布仍未授權 |
