...
TASK_LABELS = {
    ...
    "zh-TW": {
        ...
        "folder_check": "MissionCenter 資料夾建立完成",
        "hub": "視覺 HUB 連結",
        "tree": "任務樹與依賴整理",
        "bugloop": "重複除 BUG 流程",
        "smoke": "Smoke tests",
...
def main() -> int:
    ...
    lines = [
        ...
        *table_header(labels["columns"]),
        ...
        f"| {args.prefix}-P0-1 | {labels['folder_check']} | Task | {args.prefix}-E1 | P0 | Ready |  |  | {labels['create']} | bootstrap script run | 1 | intake |  |",
        f"| {args.prefix}-P0-2 | {labels['hub']} | Task | {args.prefix}-E1 | P0 | Ready |  | {args.prefix}-P0-1 | {labels['create']} | hub file visible | 1 | plan |  |",
        f"| {args.prefix}-P0-3 | {labels['tree']} | Task | {args.prefix}-E1 | P0 | Ready |  | {args.prefix}-P0-2 | {labels['seed']} | task tree visible | 2 | plan |  |",
        f"| {args.prefix}-P0-4 | {labels['bugloop']} | Task | {args.prefix}-E1 | P0 | Ready |  | {args.prefix}-P0-3 | {labels['split']} | fixed after review | 2 | execution |  |",
        f"| {args.prefix}-P0-5 | {labels['smoke']} | Task | {args.prefix}-E1 | P0 | Ready |  | {args.prefix}-P0-4 | {labels['add']} | smoke tests recorded | 2 | verification |  |",
        f"| {args.prefix}-T1 | {labels['intake']} | Task | {args.prefix}-E1 | P1 | Backlog |  | {args.prefix}-P0-5 | {labels['ask']} | {labels['checklist']} | 2 | intake |  |",
