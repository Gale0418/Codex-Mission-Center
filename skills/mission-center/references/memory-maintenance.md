...
- `brief.md` is a disposable materialized view of project identity, current work, today's journal, and active guardrail IDs.
- `focus.md` is a disposable materialized view containing only unfinished `P0` tasks from `tasks.md`.
...
2. When fresh, read `brief.md`, then `focus.md`; open `guardrails.md` when the work matches a listed situation.
...
5. When stale or truncated, run `mission_maintenance.py . sync` and fall back to the canonical files.
...
- Treat `brief.md` and `focus.md` as rebuildable caches. Do not edit them directly.
